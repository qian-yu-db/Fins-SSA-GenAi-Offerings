...
spark.sql(f'CREATE VOLUME IF NOT EXISTS {volume_name_docs};')
...
volume_folder_docs = f"/Volumes/{catalog}/{db}/{volume_name_docs}"
policy_sub = "Policies"
...
if reset_all_data:
    try:
        ...
        dbutils.fs.rm(f'{volume_folder_speech}/{transcript_sub}', True)
        dbutils.fs.rm(f'{volume_folder_docs}/{pdf_sub}', True)
        dbutils.fs.rm(f'{volume_folder_docs}/{img_sub}', True)
...
if is_folder_empty(f"{volume_folder_policy}/{policy_sub}") or is_folder_empty(f"{volume_folder_speech}/{audio_sub}"):
    ...
    download_file_from_git(dest=f'{volume_folder_docs}/{pdf_sub}', 
                           owner=repo_owner, 
                           repo=repo_name,
                           path="/datasets/pdfs")
    
    download_file_from_git(dest=f'{volume_folder_docs}/{img_sub}', 
                        owner=repo_owner, 
                        repo=repo_name,
                        path="/datasets/imgs")
